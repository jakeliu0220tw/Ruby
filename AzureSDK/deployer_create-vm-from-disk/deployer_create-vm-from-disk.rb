...
class Deployer
  ...
  def initialize(subscription_id, resource_group, resource_group_location)
    raise ArgumentError.new("Missing template file 'template_create-vm-from-disk.json' in current directory.") unless File.exist?('template_create-vm-from-disk.json')
    raise ArgumentError.new("Missing parameters file 'parameters_create-vm-from-disk.json' in current directory.") unless File.exist?('parameters_create-vm-from-disk.json')
  ...
  def deploy
    ...
    template = File.read(File.expand_path(File.join(__dir__, 'template_create-vm-from-disk.json')))
    ...
    deploy_params = File.read(File.expand_path(File.join(__dir__, 'parameters_create-vm-from-disk.json')))
    ...
    param_json["parameters"]["virtualMachineName"]["value"] = "#{@resource_group}"
    param_json["parameters"]["virtualNetworkName"]["value"] = "#{@resource_group}"
    param_json["parameters"]["networkInterfaceName"]["value"] = "#{@resource_group}"
    param_json["parameters"]["networkSecurityGroupName"]["value"] = "#{@resource_group}"
    param_json["parameters"]["publicIpAddressName"]["value"] = "#{@resource_group}"
    param_json["parameters"]["osDisk"]["value"] = "#{@resource_group}_OsDisk"
    param_json["parameters"]["dataDisk"]["value"] = "#{@resource_group}_DataDisk"
    puts param_json["parameters"]

    deployment.properties.parameters = param_json["parameters"]

    # put the deployment to the resource group
    @client.deployments.create_or_update(@resource_group, "#{@resource_group}_create-vm-from-disk", deployment)
  end
end
